...
def processVideoCallBack():
    thread = threading.Thread(target=p.processVideo, 
        args=(txVideoPath.get(),txBlBoxPath.get(),txOffset1.get(),txOffset2.get(), txTime1.get(), txTime2.get(),thread_queue))
...
lbStatus = Label(top, text = "Status:").place(x = 20, y = 220)
...
btProcess = Button(top, width=15, text ="Process video", command = processVideoCallBack).place(x = 20, y = 175)
